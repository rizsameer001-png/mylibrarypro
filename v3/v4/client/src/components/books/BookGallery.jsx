...
import { ArrowUpTrayIcon, StarIcon, TrashIcon, Bars3Icon, CheckIcon } from '@heroicons/react/24/outline';
...
export default function BookGallery({ bookId, images: initImages, onChange }) {
  ...
  const handleDelete = async (imgId) => {
    try {
      ...
      refresh(images.filter(i => i._id !== imgId));
  ...
  const saveLabel = async (img) => {
    ...
    try {
      await api.put(`/books/${bookId}/gallery/reorder`, {
        order: images.map(i => ({ _id: i._id, order: i.order, isPrimary: i.isPrimary, label: i._id === img._id ? label : i.label })),
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <div onDragOver={e => e.preventDefault()} onDrop={onDrop}
        ...
        onClick={() => document.getElementById(`gallery-upload-${bookId}`)?.click()}>
        ...
        <input id={`gallery-upload-${bookId}`} type="file" multiple accept="image/*" className="hidden"
      ...
      {images.length === 0 ? (
        <div className="text-center py-8 text-gray-400">
          <img src={PLACEHOLDER} alt="No cover" className="h-24 mx-auto opacity-40 mb-2" />
          <p className="text-sm">No images yet — upload above</p>
        </div>
      ) : (
          ...
          {images.map((img, i) => (
            <div key={img._id}
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-between">
                <button onClick={() => handlePrimary(img._id)} title="Set as primary" className="text-white hover:text-yellow-400 transition-colors">
                  {img.isPrimary ? <StarSolid className="h-4 w-4 text-yellow-400" /> : <StarIcon className="h-4 w-4" />}
                </button>
                <button onClick={() => handleDelete(img._id)} className="text-white hover:text-red-400 transition-colors">
              ...
              <div className="px-2 py-1.5 bg-white border-t border-gray-100">
                {editLabel[img._id] !== undefined ? (
                  <div className="flex items-center gap-1">
                    <input className="text-xs border border-gray-300 rounded px-1.5 py-0.5 flex-1 focus:outline-none focus:ring-1 focus:ring-primary-500"
                      value={editLabel[img._id]}
                      onChange={e => setEditLabel(prev => ({ ...prev, [img._id]: e.target.value }))}
                      autoFocus />
                    <button onClick={() => saveLabel(img)} className="text-green-600">
                      <CheckIcon className="h-4 w-4" />
                    </button>
                  </div>
                ) : (
                  <p className="text-xs text-gray-500 cursor-pointer hover:text-primary-600 truncate"
