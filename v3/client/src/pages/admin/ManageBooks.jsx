...
export default function ManageBooks() {
  ...
  useEffect(() => {
    api.get('/categories')
      ...
      .catch(e => { console.error('Categories load failed:', e); toast.error('Failed to load categories'); });
    api.get('/authors')
      .then(({data}) => setAuthors(data.authors || []))
      .catch(e => { console.error('Authors load failed:', e); toast.error('Failed to load authors'); });
    api.get('/publishers')
      .then(({data}) => setPublishers(data.publishers || []))
      .catch(e => console.error('Publishers load failed:', e));
  }, []);
  ...
  const openCreate = () => {
    setEditBook(null); setForm(EMPTY); setCoverFile(null); setEbookFile(null); setModalOpen(true);
  };
  const openEdit = (book) => {
    setEditBook(book);
    setForm({
  ...
  const toggleMulti = (key,id) => setForm(prev=>({
    ...prev, [key]: prev[key].includes(id) ? prev[key].filter(x=>x!==id) : [...prev[key],id]
  }));
  ...
  return (
    <div>
      ...
      {loading ? <LoadingSpinner /> : books.length===0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-x-auto shadow-sm">
          <table className="table-base">
            <thead><tr>
              {['Title','Authors','Category','Type','Copies','Price','Actions'].map(h=>(
                ...
                <tr key={book._id} className="hover:bg-gray-50">
                  <td className="td font-medium max-w-xs">
                    <div className="flex items-center space-x-2">
                      {book.coverImage
                        ? <img src={book.coverImage} alt="" className="h-10 w-7 object-cover rounded flex-shrink-0" />
                        : <div className="h-10 w-7 bg-gray-100 rounded flex-shrink-0 flex items-center justify-center">
                            <BookOpenIcon className="h-4 w-4 text-gray-300" />
                          </div>
                      }
                      <div>
                        <div className="truncate max-w-[160px]">{book.title}</div>
                        {book.isbn && <div className="text-xs text-gray-400">{book.isbn}</div>}
                      </div>
                    </div>
                  </td>
                  <td className="td text-xs text-gray-500">{book.authors?.map(a=>a.name).join(', ')||'—'}</td>
                  <td className="td text-xs">{book.categories?.map(c=>c.name).join(', ')||'—'}</td>
                  <td className="td">
                    <span className={typeBadge(book.bookType)}>{book.bookType}</span>
                    {book.ebookFormat && <span className="ml-1 badge bg-gray-100 text-gray-600 uppercase text-xs">{book.ebookFormat}</span>}
                  </td>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-1">
              {[
                { value:'physical', label:'Physical', icon:BuildingLibraryIcon, desc:'Physical copies only' },
                { value:'digital',  label:'Digital',  icon:DocumentIcon,        desc:'PDF/EPUB/MOBI only' },
          ...
          {isPhysical && (
            <div className="bg-blue-50 rounded-xl p-4">
              <h3 className="text-sm font-semibold text-blue-800 mb-3 flex items-center space-x-2">
                <BuildingLibraryIcon className="h-4 w-4"/><span>Physical Copy Settings</span>
              </h3>
              <div>
          ...
          {isDigital && (
            <div className="bg-purple-50 rounded-xl p-4 space-y-4">
              ...
              <div>
                ...
                {editBook?.cloudinarySecureUrl && !ebookFile && (
                  <p className="text-xs text-green-600 mt-1">✅ Digital file already uploaded via Cloudinary</p>
                )}
              </div>

              {/* Access toggles */}
              <div className="bg-white rounded-lg p-3 space-y-1 border border-purple-100">
                ...
                {form.readingEnabled && (
                  <div className="pl-2 pb-2">
                    ...
                    <select className="input text-sm" value={form.readingAccessLevel} onChange={e=>set('readingAccessLevel',e.target.value)}>
                ...
                <Toggle label="Enable Download" description="Members can download the file"
                  value={form.downloadEnabled} onChange={v=>set('downloadEnabled',v)}/>
              </div>

              {/* Sale settings */}
              <div className="bg-white rounded-lg p-3 space-y-1 border border-purple-100">
                <Toggle label="Sell as paid download" description="Set a price for purchase"
                  ...
                  <div className="pl-2 pb-2 space-y-3">
                    <div>
                      ...
                      <div className="relative max-w-[160px]">
                        ...
                        <input type="number" step="0.01" min="0" className="input pl-7" value={form.digitalPrice}
                          onChange={e=>set('digitalPrice',parseFloat(e.target.value)||0)}/>
                      </div>
                    </div>
                    <Toggle label="PDF Watermarking" description="Add buyer email to every page"
                      value={form.watermarkEnabled} onChange={v=>set('watermarkEnabled',v)}/>
                  </div>
                )}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-3 mt-1">
              {(coverFile || editBook?.coverImage) && (
                <img src={coverFile ? URL.createObjectURL(coverFile) : editBook.coverImage}
                  alt="Cover preview" className="h-20 w-14 object-cover rounded-lg border border-gray-200"/>
              )}
              <label className="btn-secondary text-sm cursor-pointer flex items-center space-x-2">
                <ArrowUpTrayIcon className="h-4 w-4"/>
              ...
              {coverFile && <button type="button" onClick={()=>setCoverFile(null)} className="text-xs text-red-500">Remove</button>}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {form.categories.length > 0 && (
                <span className="text-xs text-primary-600">{form.categories.length} selected</span>
              )}
            </div>
            {categories.length === 0 ? (
              <div className="flex items-center space-x-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <span className="text-yellow-600 text-xs">⚠️ No categories found. Go to</span>
                <a href="/admin/categories" target="_blank" className="text-xs text-primary-600 underline font-medium">
                  Manage Categories
                </a>
                <span className="text-yellow-600 text-xs">to add some.</span>
              </div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {categories.map(c=>(
                  <button key={c._id} type="button" onClick={()=>toggleMulti('categories',c._id)}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${form.categories.includes(c._id)?'bg-primary-600 text-white border-primary-600 shadow-sm':'border-gray-300 text-gray-600 hover:border-primary-400 hover:bg-primary-50'}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {form.authors.length > 0 && (
                <span className="text-xs text-primary-600">{form.authors.length} selected</span>
              )}
            </div>
            {authors.length === 0 ? (
              <div className="flex items-center space-x-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <span className="text-yellow-600 text-xs">⚠️ No authors found. Go to</span>
                <a href="/admin/categories" target="_blank" className="text-xs text-primary-600 underline font-medium">
                  Manage Categories
                </a>
                <span className="text-yellow-600 text-xs">(Authors tab) to add some.</span>
              </div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {authors.map(a=>(
                  <button key={a._id} type="button" onClick={()=>toggleMulti('authors',a._id)}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${form.authors.includes(a._id)?'bg-primary-600 text-white border-primary-600 shadow-sm':'border-gray-300 text-gray-600 hover:border-primary-400 hover:bg-primary-50'}`}>
                    {a.name}
                  </button>
                ))}
              </div>
      ...
      <ConfirmDialog open={!!deleteId} onClose={()=>setDeleteId(null)} onConfirm={handleDelete}
        title="Delete Book" message="Delete this book permanently? This also removes its Cloudinary files." danger/>
